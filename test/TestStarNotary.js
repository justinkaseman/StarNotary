...
it("can add the star name and star symbol properly", async () => {
  ...
  await instance.createStar("BIG STAR", 10, { from: owner });
...
it("lets 2 users exchange stars", async () => {
  ...
  const starId1 = 6;
  const starId2 = 9;
  await instance.createStar("First star", starId1, { from: owner });
...
it("lets a user transfer a star", async () => {
  ...
  const user2 = accounts[2];
  const starId = 7;
  await instance.createStar("The star", starId, { from: owner });
...
it("lookUptokenIdToStarInfo test", async () => {
  const instance = await StarNotary.deployed();
  const starId = 33;
  const name = "Named star";
  await instance.createStar(name, starId, { from: owner });
  const nameLookup = await instance.lookUptokenIdToStarInfo(starId);
  assert.equal(name, nameLookup);
});
